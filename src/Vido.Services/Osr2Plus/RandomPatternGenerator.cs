...
public class RandomPatternGenerator
{
    ...
    /// <param name="seed">Optional RNG seed for deterministic testing. <c>null</c> uses default RNG.</param>
    ...
    /// <summary>
    /// Updates the min/max output range (e.g. when axis config changes).
    /// </summary>
    /// <param name="min">New minimum output value.</param>
    /// <param name="max">New maximum output value.</param>
    ...
    /// <summary>
    /// Gets the cosine-interpolated position at the given progress value.
    /// Progress should be cumulative stroke distance (synced mode) or
    /// cumulative time in arbitrary units (independent mode).
    /// </summary>
    /// <param name="progress">Cumulative progress value (monotonically increasing).</param>
    ...
    /// <summary>
    /// Resets state (e.g. when playback restarts or scripts change).
    /// </summary>
    public void Reset()
    {
        _initialized = false;
