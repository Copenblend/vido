...
internal sealed class PulseBeatSource : IExternalBeatSource
{
    ...
    /// <inheritdoc />
    public void RenderBeat(SKCanvas canvas, float centerX, float centerY, float size, float progress)
    {
        ...
        if (progress > 0.05f)
        {
            DrawHeartGlow(canvas, centerX, centerY, scaledSize, progress);
        }

        // Filled heart (no stroke)
        _fillPaint.Color = HeartFill;
        DrawHeart(canvas, centerX, centerY, scaledSize, _fillPaint);
    }

    /// <inheritdoc />
    public void RenderIndicator(SKCanvas canvas, float centerX, float centerY, float size)
    {
        ...
        if (progress > 0.05f)
        {
            DrawHeartGlow(canvas, centerX, centerY, scaledSize * 1.15f, progress);
        }

        // White-stroked hollow heart — scales with hit
        _indicatorStrokePaint.StrokeWidth = 2.5f + progress * 1.5f;
        DrawHeart(canvas, centerX, centerY, scaledSize, _indicatorStrokePaint);
