...
public sealed class PlaylistProvider : IPlaylistProvider
{
    ...
    /// <summary>
    /// Initializes a new <see cref="PlaylistProvider"/> with the default random generator.
    /// </summary>
    public PlaylistProvider() : this(new Random()) { }

    /// <summary>
    /// Initializes a new <see cref="PlaylistProvider"/> with a specific <see cref="Random"/>
    /// instance for deterministic testing.
    /// </summary>
    /// <param name="random">The random number generator to use. Must not be <c>null</c>.</param>
    ...
    /// </summary>
    /// <param name="items">The playlist items collection. Must not be <c>null</c>.</param>
    /// <param name="index">The starting item index within the collection.</param>
    ...
    /// </summary>
    /// <param name="index">The new current item index.</param>
    ...
    /// <summary>
    /// Returns <c>true</c> if the file has a supported video extension.
    /// Uses <see cref="FileNode.VideoExtensions"/> for the canonical extension list.
    /// </summary>
    /// <param name="filePath">The file path to test.</param>
    /// <returns><c>true</c> if the extension is a recognized video format.</returns>
    internal static bool IsVideoFile(string filePath)
    {
        var ext = Path.GetExtension(filePath);
        return !string.IsNullOrEmpty(ext) && FileNode.VideoExtensions.Contains(ext);
    }
    ...
    /// Returns <c>null</c> if no video file is found after checking all items.
    /// </summary>
