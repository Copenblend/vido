...
/// Persists window duration via <see cref="ISettingsService"/>.
/// </summary>
internal sealed class WaveformViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    /// <param name="settingsService">Settings service for persisting window duration preference.</param>
    public WaveformViewModel(PulseEngine engine, ISettingsService settingsService)
    {
        ...
        // Load persisted window duration
        int persistedDuration = _settingsService.Current.PulseWaveformWindowDuration;
        _windowDurationSeconds = persistedDuration > 0 ? persistedDuration : 30.0;
    ...
    public double WindowDurationSeconds
    {
        ...
        set
        {
            ...
            OnPropertyChanged();
            _settingsService.Current.PulseWaveformWindowDuration = (int)value;
