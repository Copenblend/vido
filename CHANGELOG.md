...
- **vido-265**: Synchronized audio/video start with deferred WASAPI playback. Added `ArmDeferredStart()` to `AudioRenderer` that arms a deferred flag and clears the buffer; the next `SubmitSamples` call triggers `WasapiOut.Play()` automatically after priming the buffer with decoded audio. Replaced the immediate `_audioRenderer.Play()` in `FFmpegVideoEngine.Play()` with `_audioRenderer.ArmDeferredStart()`, closing the ~100ms gap between video frames appearing and audio actually playing. Added 7 tests.
