...
- **vido-213**: Removed all Pulse integration code from `MainWindow.xaml.cs` (~200 lines). Deleted 8 Pulse field declarations, `SetupPulse()` method, `SetupPulseToolbarButton()` method, `UpdatePulseToolbarIcon()` method, 3 Pulse event handlers, Pulse cleanup block from `OnClosing()`, `ExternalAxisPositionsEvent` subscription, `ConfirmOverwriteAsync()` helper, and 3 Pulse-related `using` statements.
- **vido-214**: Removed Pulse dead code from `TCodeService.cs`. Deleted external axis position fields (`_extPosValues`, `_extPosSet`, `_hasExternalPositions`, `AxisCount`), `SetExternalPositions()` method, `ClearExternalPositions()` method, `AxisOrdinal()` helper, external-priority L0 stroke tracking branch, external axis bypass in `OutputTick()`, and `hasExternalPositions` from the output loop condition. Removed 12 related tests from `TCodeEngineTests.cs` (AxisOrdinal, SetExternalPositions/ClearExternalPositions, external stroke tracking integration tests).
- **vido-215**: Removed Pulse dead code from `AxisControlViewModel`, `BeatBarViewModel`, and `BeatBarOverlay`. Deleted `_funscriptsSuppressed` field, `OnSuppressFunscript()` method, `IsFunscriptsSuppressed` property, and suppression guard from `LoadScriptsForVideo()` in `AxisControlViewModel`. Deleted external beat source fields (`_externalSources`, `_externalBeats`, `_pendingExternalModeId`, `_preExternalMode`, `_savedExternalModeId`), `ActiveExternalSource` property, `IsExternalMode` property, `OnBeatSourceRegistration()` method, `OnExternalBeatEvent()` method, `RebuildAvailableModes()` method, external mode tracking in `Mode` setter, deferred external mode logic in `LoadSettings()`, `PersistFallbackMode()`, `CopyBeatTimesToExternalBeats()`, and `HasExternalSource()` helpers from `BeatBarViewModel`. Deleted `PaintExternalSource()` method and external source rendering delegation from `BeatBarOverlay`. Removed 3 EventBus subscriptions (`ExternalBeatSourceRegistration`, `ExternalBeatEvent`, `SuppressFunscriptEvent`) from `MainWindow.xaml.cs`. Removed 22 related tests from `Osr2ViewModelTests.cs`.
- **vido-216**: Deleted 10 Pulse-only test files (500 tests): `PulseAudioServiceTests.cs`, `PulseRealtimeServiceTests.cs`, `PulseModelTests.cs`, `PulseSidebarViewModelTests.cs`, `PulseSidebarViewModelStrokeControlTests.cs`, `PulseViewModelTests.cs`, `PulseStrokeControlModelTests.cs`, `PulseTCodeMapperStrokeControlTests.cs`, `FunscriptWriterTests.cs`, `FunscriptWriterStrokeControlTests.cs`.
- **vido-209**: Deleted all Pulse source directories and files: `src/Vido.Views/Pulse/` (6 files), `src/Vido.ViewModels/Pulse/` (2 files), `src/Vido.Services/Pulse/` (13 files), `src/Vido.Core/Models/Pulse/` (7 files), `src/Vido.Views/Assets/Pulse/` (2 files), `src/Vido.Services/Osr2Plus/FunscriptWriter.cs`, `src/Vido.Views/Themes/PulseStyles.xaml`. Removed PulseStyles.xaml ResourceDictionary merge from `App.xaml` and 2 Pulse asset Resource entries from `Vido.Views.csproj`. Deleted Pulse-only test file `WaveformStripRendererTests.cs`.
- **vido-210**: Deleted 6 Pulse-only types from `Core/Haptics/` (`AxisPosition.cs`, `ExternalAxisPositionsEvent.cs`, `ExternalBeatEvent.cs`, `ExternalBeatSourceRegistration.cs`, `IExternalBeatSource.cs`, `SuppressFunscriptEvent.cs`) and `FunscriptGeneratedEvent.cs` from `Core/Events/`. Removed 14 Pulse-only test methods from `HapticTypesTests.cs` (AxisPosition, ExternalAxisPositionsEvent, ExternalBeatEvent, ExternalBeatSourceRegistration, SuppressFunscriptEvent, IExternalBeatSource). Cleaned up stale `IExternalBeatSource` doc comment reference in `BeatBarMode.cs`.
- **vido-211**: Removed 10 `Pulse*` properties from `AppSettings.cs` (`PulseBeatSensitivity`, `PulseEnableBpmPhaseLock`, `PulseWaveformWindowDuration`, `PulseUsePulse`, `PulseBeatRateIndex`, `PulseFunscriptBeatRateIndex`, `PulseAmplitudeOffset`, `PulseEasingBlend`, `PulseStrokePattern`, `PulseRandomness`) and their 10 `ResetToDefaults()` assignments. Removed 3 Pulse default tests and Pulse assertions from `ResetToDefaults` test in `FeatureSettingsTests.cs`. Cleaned up stale Pulse doc comment in `Osr2BeatBarFallbackMode`.
...
- **vido-198**: Extended `PulseSidebarViewModel` with four stroke control properties: `AmplitudeOffset` (-1.0 to +1.0, clamped), `EasingBlend` (-1.0 to +1.0, clamped), `SelectedStrokePattern`/`SelectedStrokePatternIndex` (enum/ComboBox mapping), and `Randomness` (0.0 to 1.0, clamped). Each property persists to `AppSettings`, calls `QueueSave()`, and propagates to `PulseEngine.SetStrokeSettings()` via `BuildStrokeSettings()` helper. Added display properties `AmplitudeOffsetLabel` (format "+0.0;-0.0;0.0"), `RandomnessLabel` (percentage), and static `StrokePatternOptions` list. Constructor loads all values from `AppSettings` with `Enum.TryParse` fallback for invalid pattern strings. Updated `GenerateFunscriptAsync` to pass `BuildStrokeSettings()` to `FunscriptWriter.CreateActionsFromBeatMap`. Updated `Description` text to document stroke controls and funscript generation behavior. 27 new tests, 83.28% file coverage (all new code fully covered).
- **vido-197**: Added `StrokeSettings` property and `SetStrokeSettings(PulseStrokeSettings?)` method to `PulseEngine`. Propagates stroke settings to `PulseTCodeMapper` inside the lock and resets the mapper to avoid jarring transitions. Null input falls back to `PulseStrokeSettings.Default`. 6 new unit tests, 100% coverage on new code.
- **vido-196**: Extended `FunscriptWriter` with new `CreateActionsFromBeatMap(BeatMap, PulseStrokeSettings)` overload that bakes stroke controls into generated funscript actions. Supports amplitude offset (same formula as PulseTCodeMapper), per-beat randomness (deterministic xorshift32), and five stroke patterns: Classic (1 action/beat, unchanged), DoubleTap (4 keyframes/beat), TripleTap (6 keyframes/beat), HoldTop (3 keyframes with 30%/70%/100% timing), HoldBottom (inverted hold). Existing parameterless overload delegates to new method with `PulseStrokeSettings.Default` for full backward compatibility. 99.32% coverage with 43 new tests (71 total for FunscriptWriter).
